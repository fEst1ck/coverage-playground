...
#[derive(Default)]
pub struct NGramCoverage {
    edges: FxHashMap<(u32, u32, u32, u32), usize>,
}

impl CoverageMetric for NGramCoverage {
    fn update_from_path(&mut self, path: &[u32]) -> CoverageFeedback {
        let mut new_coverage = false;

        let mut uniq = usize::MAX;
        for window in path.windows(4) {
            let edge = (window[0], window[1], window[2], window[3]);
